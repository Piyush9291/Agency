...
export default function AllServices() {
  ...
  return (
    <>
      ...
      />
      <div className="bg-black/20 pt-20" data-testid="all-services-page">
      {/* Hero Section */}
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {services.map((service, idx) => (
              ...
              >
                ...
                {/* Card */}
                <div className="relative h-full p-8 rounded-3xl bg-white/10 backdrop-blur-xl border border-purple-500/50 hover:border-cyan-500/50 transition-all duration-300 backdrop-blur-sm">
                  {/* Icon */}
                  ...
                  <p className="text-white0 font-jakarta text-sm mb-4">{service.subtitle}</p>
                  ...
                  {/* Features List */}
                  <div className="mb-6">
                    <h4 className="text-white font-syne font-semibold mb-3 text-sm uppercase tracking-wide">
                      What's Included:
                    </h4>
                    <ul className="space-y-2">
                      {service.features.map((feature, fIdx) => (
                        <li key={fIdx} className="flex items-start text-gray-400 font-jakarta text-sm">
                  ...
                  <Link to="/submit-project">
                    <button className="w-full rounded-full px-6 py-3 bg-purple-500/10 hover:bg-white/10 backdrop-blur-xl/10 border border-purple-500/50 hover:border-cyan-500/50 transition-all duration-300 font-semibold font-jakarta flex items-center justify-center gap-2 group">
                      Get Started
      ...
      <section className="py-24 px-6 md:px-12 bg-purple-500/100">
        <div className="max-w-7xl mx-auto">
          ...
          >
            ...
            <p className="text-gray-400 font-jakarta text-lg max-w-3xl mx-auto">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {[
              ...
              <motion.div
                ...
                viewport={{ once: true }}
                transition={{ duration: 0.6, delay: idx * 0.1 }}
                className="text-center p-8 rounded-2xl bg-white/10 backdrop-blur-xl border border-purple-500/50 hover:border-cyan-500/30 transition-all duration-300"
              >
                <div className="text-5xl mb-4">{item.icon}</div>
                <h3 className="text-xl font-bold font-syne mb-3">{item.title}</h3>
                <p className="text-gray-400 font-jakarta">{item.desc}</p>
              </motion.div>
            ))}
          </div>
        </div>
              ...
              <div className="flex flex-col sm:flex-row items-center justify-center gap-6">
                ...
                <Link to="/contact">
                  <motion.button
                    whileHover={{ scale: 1.05 }}
                    whileTap={{ scale: 0.95 }}
                    className="rounded-full px-10 py-5 border-2 border-white/20 hover:bg-white/10 backdrop-blur-xl/10 transition-colors duration-200 font-bold font-jakarta text-lg"
                  >
                    Talk to an Expert
                  </motion.button>
                </Link>
