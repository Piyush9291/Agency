              ...
              <motion.div
                ...
                className={`relative flex flex-col p-8 rounded-3xl border transition-all duration-300 ${
                  plan.popular
                    ...
                    : "bg-gray-500 border-gray-200/50 hover:border-indigo-500/30"
                }`}
                data-testid={`pricing-plan-${idx}`}
              >
                {plan.popular && (
                  <div className="absolute -top-4 left-1/2 transform -translate-x-1/2">
                    <span className="px-6 py-2 rounded-full bg-gradient-to-r from-purple-600 to-teal-500 text-gray-900 font-jakarta text-sm font-semibold">
                      Most Popular
                    </span>
                  </div>
                )}
          ...
          <div className="grid md:grid-cols-2 gap-6">
            {addOns.map((addon, idx) => (
              <motion.div
                ...
                className="flex items-center justify-between p-6 rounded-2xl bg-white border border-gray-200/50 hover:border-indigo-500/30 transition-all duration-300"
